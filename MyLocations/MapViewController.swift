...
class MapViewController: UIViewController {
    ...
    // MARK:- Actions
    // Zooms into user on the map
    @IBAction func showUser() {
        let region = MKCoordinateRegion(center: mapView.userLocation.coordinate, latitudinalMeters: 1000, longitudinalMeters: 1000)
        mapView.setRegion(mapView.regionThatFits(region), animated: true)
    }
    
    // Groups locations and shows all of them on a single screen
    ...
    // Region grouping for the locations button
    ...
    // Sets up segue to the edit locations view
    @objc func showLocationDetails(_ sender: UIButton) {
        performSegue(withIdentifier: "EditLocation", sender: sender)
    }
    
    // MARK:- Navigation
    // For editing location details from the map view
...
// MARK:- Extensions
// Properties for the Map pins and button
